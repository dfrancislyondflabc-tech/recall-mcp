...
for (const p of [indexPath(), stagingIndexPath()].filter(Boolean)) {
  ...
  for (const d of idx.docs || []) {
    ...
    if (Array.isArray(d.summaryVec) && d.name) {
      cacheSet(cache, `${String(d.name).replace(/[-_]/g, ' ')}. ${d.description || ''}`, d.summaryVec);
      summaries++;
    }
    for (const c of d.chunks || []) {
      if (c && typeof c.text === 'string' && Array.isArray(c.vec)) { cacheSet(cache, c.text, c.vec); chunks++; }
    }
  }
