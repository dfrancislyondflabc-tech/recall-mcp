...
import { readFileSync, writeFileSync, existsSync, statSync, renameSync, unlinkSync, mkdirSync, openSync, readSync, closeSync, appendFileSync } from 'node:fs';
...
export function indexBuiltAtOnDisk(out) {
  try {
    ...
    try {
      const buf = Buffer.alloc(4096);
      const n = readSync(fd, buf, 0, 4096, 0);
      const m = buf.toString('utf8', 0, n).match(/"builtAt"\s*:\s*"([^"]+)"/);
      return m ? m[1] : null;
    } finally { closeSync(fd); }
  } catch { return null; }
}
