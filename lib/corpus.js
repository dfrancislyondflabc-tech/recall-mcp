...
import { memoryDir, memoryRoots } from './config.js';
...
export function parseFrontmatter(raw) {
  if (!raw.startsWith('---')) return { front: null, body: raw, endIndex: 0 };
  const end = raw.indexOf('\n---', 3);
  if (end === -1) return { front: null, body: raw, endIndex: 0 };
  const block = raw.slice(3, end);
  const afterIdx = raw.indexOf('\n', end + 1) + 1;
  const body = raw.slice(afterIdx);
...
export function listCorpusFiles(dirOrRoots = memoryRoots()) {
  ...
  for (const root of roots) {
    ...
    for (const file of files) {
      // The DENYLIST and the section-scrub key on the BARE filename, so they
      // must never see the namespaced id — otherwise a second root silently
      // disarms both of them.
      out.push({
        file,
...
export function loadCorpus(dirOrRoots = memoryRoots()) {
  ...
  return { docs, excluded, dir: roots[0]?.dir, roots, sectionReport };
}
