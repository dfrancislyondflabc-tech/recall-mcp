...
export function readFilesAt(p, { maxDepth = 4, structure = false } = {}) {
  ...
  let realRoot;
  try { realRoot = realpathSync(p); } catch { realRoot = null; }
  const escapes = (full) => {
    if (!realRoot) return false;
    try {
      // lstat first: only a symlink can escape, and resolving every plain file would cost a
      // syscall per entry for nothing.
      ...
      return true;      // unresolvable symlink — refuse it rather than guess
    }
  };
