  ...
  if (idx.dense) {
    ...
    if (qvec) {
      ...
      docs.forEach((doc, i) => {
        ...
        if (Array.isArray(doc.summaryVec)) {
...
function absenceVerdict({ results, topPhrase, rawKw, stats, mode, aliasFreeScore = null, scope = null }) {
  ...
  return {
    ...
    absenceNote: readThisFirst
      ...
      ? `READ BEFORE JUDGING: a document scored well here (${topScore.toFixed(3)}) but in different ` +
        `words from yours — ${why}. A strong score with none of your words is what BOTH an answer in ` +
        'other language and an adjacent-but-different topic look like; the text decides, and nothing ' +
        'else can. OPEN bestWeak[0] and read it. If it answers the question, answer and cite it. If it ' +
        'turns out to be about something adjacent but different, say the memory is absent — that ' +
        'judgement needs the text, and no score can make it for you. Do not report a neighbour as a ' +
        'memory of this without reading it.'
...
export async function nearest(doc, docs, n = 3) {
  const vecs = (doc.chunks || []).map((c) => c.vec).filter(Array.isArray);
...
export async function latest(query, opts = {}) {
  ...
  for (const doc of idx.docs) {
    ...
    for (const t of matched) df.set(t, df.get(t) + 1);
  ...
  if (hits.length) {
    guidance.push('Ordered NEWEST FIRST by ' + (corpusClock(idx) === 'ts' ? 'when it was said' : 'FILE MTIME (not chronology)') +
