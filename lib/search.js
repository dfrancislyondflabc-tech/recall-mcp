...
async function ensureFresh(scope) {
  ...
  // files). Both timestamps must parse before anything happens, so an unreadable or headerless
  ...
  if (onDiskBuiltAt && loadedBuiltAt) {
    const a = Date.parse(onDiskBuiltAt), b = Date.parse(loadedBuiltAt);
  ...
  if (reloadedFromDisk) {
    ...
    stamp.indexReloadNote = `The index was rebuilt by another process (on disk ${onDiskBuiltAt}, this ` +
